...
import sys
rootdir = "C:/Sunaoxue/IT_Project/warrantysmart/Data/Normal/txt/"
file_srx = open("train_list_N.txt")  # 其中包含所有待计算的文件名
...
while line:
    ...
    filename = filename[:-4]
    print filename
    file_object = open(rootdir + filename + ".txt", 'w') #写文件
    ...
    for object in root.findall('object'):  # 找到root节点下的所有object节点
        ...
        if name == ("bicycle" or "motorbike"):
            ...
            flag = True
        if name == ("Normal"):
            file_object.write("Normal" + " 0 0 0 " + xmin + ".00 " + ymin + ".00 " + xmax + ".00 " + ymax + ".00 " + "0 0 0 0 0 0 0" + "\n")
            # file_object_log.write(str(float(int(xmax) - int(xmin)) * 1920.0 / float(width)) + " " + str(float(int(ymax) - int(ymin)) * 1080.0 / float(height)) + "\n")
            flag = True
        if name == ("Abnormal"):
            file_object.write("Abnormal" + " 0 0 0 " + xmin + ".00 " + ymin + ".00 " + xmax + ".00 " + ymax + ".00 " + "0 0 0 0 0 0 0" + "\n")
            #file_object_log.write(str(float(int(xmax) - int(xmin)) * 1920.0 / float(width)) + " " + str(float(int(ymax) - int(ymin)) * 1080.0 / float(height)) + "\n")
            flag = True
    file_object.close( )
