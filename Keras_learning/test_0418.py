...
def main():
    ...
    Width = 128
    Height = 128
    num_classes = 2  # Caltech101为102  cifar10为10
    pic_dir_out = 'C:/Sunaoxue/IT_Project/warrantysmart/Data'
    pic_dir_data = 'C:/Sunaoxue/IT_Project/warrantysmart/Data/test_9418/'
    (X_train, y_train), (X_test, y_test) = get_data("D", 0.7, data_format='channels_last')
    # (X_train, y_train), (X_test, y_test) = get_data("D", 0.7, data_format='channels_last')
    ...
    model.fit(X_train, y_train, epochs=1000, batch_size=128, )  # 正式训练数据
    model.save_weights(os.path.join(pic_dir_out, 'cnn_model_Caltech101_' + cm2_str + '.h5'))
    model.save('my_model_0418.h5')
